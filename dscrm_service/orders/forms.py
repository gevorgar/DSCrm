...
class OrderForm(forms.Form):
    ...
    client_phone = forms.CharField(label='Номер телефона', max_length=20, widget=forms.TextInput(
        attrs={'class': 'form-control', 'type': 'text'}))
    device_group = forms.ModelChoiceField(empty_label='Выберите группу', label='Группа', queryset=Group.objects.all(),
                                          widget=forms.Select(
                                              attrs={'class': 'form-control'}))
    device_brand = forms.ModelChoiceField(empty_label='Выберите бренд', label='Бренд', queryset=Brand.objects.all(),
                                          widget=forms.Select(
                                              attrs={'class': 'form-control'}))
    device_model = forms.ModelChoiceField(empty_label='Выберите Модель', label='Модель', queryset=Model.objects.all(),
                                          widget=forms.Select(
                                              attrs={'class': 'form-control'}))
